...
describe('Parser', () => {
    beforeEach(function () {
      ...
      cy.route('POST','http://localhost:8080/parser/loadFileTree', "@loadFileTreeDefault");
      cy.route('POST','http://localhost:8080/parser/loadDocumentDescription', "@loadDocumentDescriptionDefault");

    });
  
    it('should see logo', () => {
      cy.visit('/parser');
      cy.get('#gd-header-logo .text').should('have.text', 'parser');
    });
    ...
    it('should open file dialog when clicked on open file icon', () => {
      ...
      it('should be able to open sub folder', () => {
        ...
        cy.route('POST','http://localhost:8080/parser/loadFileTree',"@loadFileTreeSubFolder");
